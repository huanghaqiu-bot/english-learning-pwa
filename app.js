...
function renderChoice(area, q) {
  // 每次渲染都打乱选项顺序
  const shuffled = _shuffleOptions(q.options, q.answer);
  ...
  if (q.hint) {
    ...
    hintText.className = "question-hint hint-hidden";
    // 遮蔽提示中的答案词，避免直接泄露
    const masked = _maskHint(q.hint, q.options[q.answer]);
    hintText.textContent = masked;
    hintBtn.addEventListener("click", () => {
      if (hintText.classList.contains("hint-hidden")) {
        hintText.classList.remove("hint-hidden");
...
/**
 * 打乱选择题选项顺序，同时跟踪正确答案的新位置
 * @param {string[]} options - 原始选项数组
 * @param {number} answer - 原始正确答案索引
 * @returns {{options: string[], answer: number}} 打乱后的选项和新的正确答案索引
 */
...
/**
 * 遮蔽提示中包含正确答案的部分，避免审题时直接泄露答案
 * 例如：hint="put up with = 忍受", answer="put" → "___ up with = 忍受"
 */
function _maskHint(hint, correctAnswer) {
  if (!hint || !correctAnswer) return hint;
  // 将正确答案从提示中替换为下划线
  let masked = hint;
  // 大小写不敏感替换
...
function renderErrorBook() {
  ...
  const dueReviews = [];
  for (const [w, d] of allErrors) {
    if (d.nextReview <= today && d.mastery < 5) dueReviews.push({ word: w, ...d });
  }
  ...
  const sorted = allErrors.sort((a, b) => {
    const aDue = a[1].nextReview <= today ? 0 : 1;
    const bDue = b[1].nextReview <= today ? 0 : 1;
    if (aDue !== bDue) return aDue - bDue;
    return b[1].count - a[1].count;
  });

  // 限制显示数量，避免大量 DOM 导致卡顿
  const MAX_DISPLAY = 50;
  const displayList = sorted.slice(0, MAX_DISPLAY);

  // 使用 DocumentFragment 批量插入，避免逐个 reflow
  const frag = document.createDocumentFragment();

  displayList.forEach(([word, data]) => {
    const isDue = data.nextReview <= today;
    const masteryStars = "★".repeat(Math.min(data.mastery, 5)) + "☆".repeat(5 - Math.min(data.mastery, 5));
    const item = document.createElement("div");
  ...
  if (sorted.length > MAX_DISPLAY) {
    const more = document.createElement("div");
    more.className = "error-more-hint";
    more.textContent = `还有 ${sorted.length - MAX_DISPLAY} 条错题，复习后会逐步显示`;
...
function showInstantAnalysis(correctWord, userAnswer, zh, isCorrect, q) {
  ...
  // 调用智能分析引擎
  const analysis = _analyzeQuestion(q, q.answer, isCorrect ? null : lessonState.selectedAnswer);

  let html = "";

  // ── 1. 错误归因标签 ──
  if (!isCorrect && analysis.errorInfo) {
    html += `<div class="ia-error-badge ia-badge-${analysis.errorInfo.color || 'blue'}">`;
    html += `<span class="ia-badge-icon">${analysis.errorInfo.icon || '💡'}</span>`;
    html += `<span class="ia-badge-label">${analysis.errorInfo.label}</span>`;
    html += `</div>`;
  }
  ...
  // ── 3. 为什么错了（针对性分析）──
  if (!isCorrect && analysis.whyWrong) {
    html += `<div class="ia-why-wrong">`;
    html += `<div class="ia-why-title">🔍 错因分析</div>`;
    html += `<div class="ia-why-text">${analysis.whyWrong}</div>`;
    html += `</div>`;
  }

  // ── 4. 逐项解析 ──
  if (!isCorrect && analysis.optionsAnalysis && analysis.optionsAnalysis.length > 0) {
    html += `<div class="ia-options-analysis">`;
    html += `<div class="ia-options-title">📋 逐项解析</div>`;
    const letters = ["A", "B", "C", "D", "E"];
    analysis.optionsAnalysis.forEach((opt, i) => {
      const cls = opt.isCorrect ? "opt-correct" : (opt.isUserChoice ? "opt-user-wrong" : "opt-other");
      const icon = opt.isCorrect ? "✓" : (opt.isUserChoice ? "✗" : "○");
      html += `<div class="ia-opt-row ${cls}">`;
      html += `<div class="ia-opt-head">`;
      html += `<span class="ia-opt-letter">${letters[i] || ""}</span>`;
      html += `<span class="ia-opt-text">${opt.text}</span>`;
      html += `<span class="ia-opt-icon">${icon}</span>`;
      html += `</div>`;
      html += `<div class="ia-opt-explain">${opt.explanation}</div>`;
      html += `</div>`;
    });
    html += `</div>`;
  }
  ...
  // ── 7. 对比学习（额外例句）──
  if (!isCorrect && analysis.compareExamples && analysis.compareExamples.length > 0) {
    html += `<div class="ia-compare-examples">`;
    html += `<div class="ia-compare-ex-title">🔄 对比学习</div>`;
    analysis.compareExamples.forEach(ex => {
      html += `<div class="ia-compare-ex-item">`;
      html += `<div class="ia-compare-ex-en">${ex.en} <button class="ia-speak-mini" onclick="speak('${ex.en.replace(/'/g, "\\'")}')">🔊</button></div>`;
      if (ex.zh) html += `<div class="ia-compare-ex-zh">${ex.zh}</div>`;
      if (ex.note) html += `<div class="ia-compare-ex-note">📌 ${ex.note}</div>`;
      html += `</div>`;
    });
    html += `</div>`;
  }
  ...
  // ── 8. 学习建议（随机轮换）──
  if (!isCorrect && analysis.studyTip) {
    html += `<div class="ia-study-tip">${analysis.studyTip}</div>`;
  }

  // ── 9. 错题本提示 ──
  if (!isCorrect) {
    html += `<div class="ia-tip">📌 已加入错题本，系统会按 <b>1天→2天→4天→7天→14天</b> 间隔提醒复习</div>`;
  }
