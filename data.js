...
const _ERROR_TYPES = [
  { pattern: /if.*were|虚拟|条件句|second conditional|subjunctive/i, label: "虚拟语气·条件句", icon: "🔷", color: "purple" },
  { pattern: /被动|passive|be.*(written|done|built|spoken|made|repaired|rescheduled)|was.*by/i, label: "被动语态", icon: "🔄", color: "blue" },
  { pattern: /定语从句|relative clause|关系代词|先行词/i, label: "定语从句", icon: "🔗", color: "teal" },
  { pattern: /现在完成|present perfect|have.*(been|done|seen|lost|finished)|has.*never/i, label: "现在完成时", icon: "⏳", color: "green" },
  { pattern: /情态|modal verb|should|could you|must not|can not|cannot/i, label: "情态动词", icon: "🔑", color: "orange" },
  { pattern: /一般现在|present simple|third person|第三人称|加.*s/i, label: "一般现在时", icon: "🔁", color: "cyan" },
  { pattern: /过去式|past tense|ed$|yesterday|last (week|month|year)/i, label: "过去时态", icon: "📜", color: "amber" },
  { pattern: /将来|future|will|going to|shall/i, label: "将来时态", icon: "🔮", color: "indigo" },
  { pattern: /动名词|gerund|enjoy.*doing|finish.*doing|practice.*doing/i, label: "动名词用法", icon: "🎬", color: "pink" },
  { pattern: /不定式|infinitive|to (do|be|go|make|take)|want.*to/i, label: "不定式", icon: "➡️", color: "lime" },
  { pattern: /介词|preposition|in.*morning|on.*day|at.*time/i, label: "介词搭配", icon: "📌", color: "red" },
  { pattern: /冠词|article|a |an |the /i, label: "冠词用法", icon: "🏷️", color: "rose" },
  { pattern: /短语动词|phrasal verb|give up|put off|break down|carry on|look forward/i, label: "短语动词", icon: "🧩", color: "violet" },
  { pattern: /搭配|collocation|make.*decision|take.*breath|heavy.*rain/i, label: "固定搭配", icon: "🎯", color: "fuchsia" },
  { pattern: /习语|idiom|piece of cake|break a leg|raining cats/i, label: "英语习语", icon: "🎭", color: "purple" },
  { pattern: /比较级|comparative|superlativ|more.*than|most|.*er than/i, label: "比较级/最高级", icon: "⚖️", color: "blue" },
  { pattern: /祈使|imperative|please.*(proceed|show|wait)/i, label: "祈使句", icon: "📢", color: "orange" },
  { pattern: /礼貌|polite|could i|would you|i'd like/i, label: "礼貌表达", icon: "🤝", color: "green" },
];

const _OPTION_ANALYSIS = {
  // 语法选项解析
  "were": "虚拟语气中 be 动词统一用 were（不论人称），表与现在事实相反的假设",
  "was": "❌ was 是过去式，但在第二条件句（虚拟语气）中应用 were",
  "am": "❌ am 是现在时，与条件句中的 would 矛盾，时态不一致",
  "will be": "❌ will be 是将来时，if 从句中不能用 will（第一条件句用一般现在时表将来）",
  "be": "❌ 原形 be 不能直接做谓语，需要配合情态动词",
  "has": "❌ 第三人称单数用 has，但此处主语是 I 应用 have",
  "have": "have/has + 过去分词构成现在完成时，表示过去发生的动作对现在有影响",
  "is": "❌ is 用于一般现在时的单数主语，此处需用被动语态 was written",
  "are": "❌ are 用于复数主语，此处 letter 是单数",
  "who": "who 引导定语从句，先行词是人，在从句中做主语",
  "which": "❌ which 引导定语从句，先行词是物，不能用于指人",
  "that": "that 可指人或物，但在非限制性定语从句（逗号后）中不能用",
  "where": "❌ where 表地点，先行词不是地点时不能用",
  "in": "check in = 办理入住/登记，固定搭配",
  "out": "❌ check out = 退房，与句意\"入住\"相反",
  "up": "❌ check up = 检查/核实，与酒店场景不符",
  "on": "❌ check on = 查看/确认，与酒店入住场景不符",
  "order": "order = 点餐，I'd like to order = 我想点...",
  "eat": "❌ eat 意为\"吃\"，但点餐时用 order 更正式",
  "cook": "❌ cook 意为\"烹饪\"，客人不会自己做饭",
  "try": "❌ try 意为\"尝试\"，点餐语境应直接说 order",
  "schedule": "schedule a meeting = 安排会议，商务常用搭配",
  "plan": "❌ plan 侧重于计划，不如 schedule 在商务中正式",
  "by": "start by doing = 以...开始，固定搭配",
  "from": "❌ from 表来源，此处应用 by 表方式",
  // 旅游英语
  "靠窗还是靠走道座位": "✓ Window = 靠窗座位，aisle = 靠走道座位",
  "头等舱还是经济舱": "❌ 头等舱 = first class，经济舱 = economy class",
  "大件还是小件行李": "❌ 行李 = luggage/baggage，不会用 window/aisle 提问",
  "是否需要转机": "❌ 转机 = connecting flight，与 window/aisle 无关",
  "您有预订吗？": "✓ reservation = 预订，Do you have a reservation? = 您有预订吗？",
  "您要预订吗？": "❌ \"您要预订\"应说 Would you like to make a reservation?",
  "您取消预订吗？": "❌ 取消预订 = cancel a reservation",
  "您想换房吗？": "❌ 换房 = change rooms",
  "直走，第二个拐角左转": "✓ go straight = 直走，turn left = 左转，second corner = 第二个拐角",
  "直走，第二个拐角右转": "❌ turn left 是左转不是右转(right)",
  "左转后直走两个路口": "❌ 语序和方向都不对",
  "在第二个路口左转再直走": "❌ 原句是先直走再左转，不是先左转再直走",
  // 商务
  "agenda": "agenda = 议程，会议常用词",
  "minutes": "minutes = 会议纪要（注意不是\"分钟\"）",
  "deadline": "deadline = 截止日期",
  "follow up": "follow up = 跟进",
};

const _COMPARE_EXAMPLES = {
  "were": [
    { en: "If I were you, I would study harder.", zh: "如果我是你，我会更努力学习。", note: "If I were you 是固定虚拟表达" },
    { en: "If she were here, she would help us.", zh: "如果她在这里，她会帮我们。", note: "she 也用 were（虚拟语气）" },
  ],
  "was": [
    { en: "He was happy yesterday.", zh: "他昨天很开心。", note: "was 用于真实过去叙述，非虚拟" },
    { en: "If I was wrong, I apologize.", zh: "如果我错了，我道歉。", note: "口语中偶尔用 was 替代 were（非正式）" },
  ],
  "have": [
    { en: "I have finished my homework.", zh: "我已经完成了作业。", note: "have + 过去分词 = 现在完成时" },
    { en: "She has never been to Paris.", zh: "她从未去过巴黎。", note: "第三人称用 has" },
  ],
  "who": [
    { en: "The man who lives next door is a doctor.", zh: "住在隔壁的男人是医生。", note: "who 指人，做主语" },
    { en: "The girl who I met yesterday called me.", zh: "我昨天遇到的女孩给我打电话了。", note: "who 指人，做宾语（可用 whom）" },
  ],
  "in": [
    { en: "I'd like to check in, please.", zh: "我想办理入住。", note: "check in = 入住登记" },
    { en: "What time is check-in?", zh: "几点可以入住？", note: "check-in 做名词时加连字符" },
  ],
  "order": [
    { en: "I'd like to order the steak.", zh: "我想点牛排。", note: "order = 点餐" },
    { en: "Are you ready to order?", zh: "您准备好点餐了吗？", note: "服务员常用问句" },
  ],
};

const _STUDY_TIPS = [
  "💡 建议把这道题加入错题本，隔1天、2天、4天后再做一遍，形成长期记忆",
  "💡 试着用自己的话复述这个语法点，能说出来才是真正掌握了",
  "💡 找3个类似的句子练习，举一反三比死记硬背更有效",
  "💡 把正确答案代入原句大声朗读3遍，培养语感",
  "💡 记住这个搭配的\"触发词\"，遇到类似结构就能快速反应",
  "💡 对比正确和错误选项的差异，重点关注它们在什么场景下会混淆",
  "💡 今晚睡前回顾这道题，睡眠有助于巩固新学的知识",
  "💡 尝试用这个知识点造一个与自己生活相关的句子",
];

function _classifyError(prompt, correctText, userText, hint) {
  const text = (prompt + " " + (hint || "")).toLowerCase();
  for (const t of _ERROR_TYPES) {
    if (t.pattern.test(text)) return t;
  }
  // 如果没有匹配，检查选项特征
  if (correctText) {
    // 如果答案是中文，说明是词义理解题
    if (/[\u4e00-\u9fa5]/.test(correctText)) return { label: "词义理解", icon: "📖", color: "blue" };
    const d = getWordDetail(correctText);
    if (d.pos === "n.") return { label: "词汇辨析", icon: "📖", color: "blue" };
    if (d.pos === "v.") return { label: "动词用法", icon: "⚡", color: "orange" };
    if (d.pos === "adj.") return { label: "形容词用法", icon: "🎨", color: "purple" };
    if (d.pos === "adv.") return { label: "副词用法", icon: "💨", color: "cyan" };
  }
  return { label: "知识点解析", icon: "💡", color: "blue" };
}

function _getOptionExplanation(prompt, option, isCorrect, q, errorLabel) {
  // 1. 对于正确答案，生成正向解析
  if (isCorrect) {
    const d = getWordDetail(option);
    if (d.ex) return `✓ 正确。${d.pos || ""} ${d.exZh || ""}`;
    if (q.hint) return `✓ 正确。提示：${q.hint}`;
    // 根据错误类型给出针对性解释
    if (errorLabel && errorLabel.includes("条件句")) return "✓ 虚拟语气中 be 动词用 were（不论人称）";
    if (errorLabel && errorLabel.includes("被动")) return "✓ 被动语态：be + 过去分词";
    if (errorLabel && errorLabel.includes("完成时")) return "✓ have/has + 过去分词 = 现在完成时";
    if (errorLabel && errorLabel.includes("定语从句")) return "✓ 正确的关系代词，与先行词匹配";
    return "✓ 这是正确答案";
  }

  // 2. 对于错误答案，先查预设解析表
  const key = option.toLowerCase().trim();
  if (_OPTION_ANALYSIS[key]) return _OPTION_ANALYSIS[key];
  if (_OPTION_ANALYSIS[option]) return _OPTION_ANALYSIS[option];

  // 3. 根据错误类型推断
  if (errorLabel && errorLabel.includes("条件句")) {
    return `❌ 虚拟语气条件句中不能用 ${option}，应用 were`;
  }
  if (errorLabel && errorLabel.includes("被动")) {
    return `❌ 被动语态需要 be + 过去分词，${option} 结构不对`;
  }
  if (errorLabel && errorLabel.includes("完成时")) {
    return `❌ 现在完成时需要 have/has + 过去分词，${option} 不符合`;
  }
  if (errorLabel && errorLabel.includes("定语从句")) {
    return `❌ 关系代词选择错误，${option} 不适合此语境`;
  }
  if (errorLabel && errorLabel.includes("搭配")) {
    return `❌ ${option} 与句中动词不构成正确搭配`;
  }

  // 4. 默认分析
  const d = getWordDetail(option);
  if (d.pos) return `❌ ${d.pos} ${option}：词义或用法不符合此处语境`;
  return "❌ 不符合此处语境";
}

function _getCompareExamples(correctText) {
  const key = correctText.toLowerCase().trim();
  if (_COMPARE_EXAMPLES[key]) return _COMPARE_EXAMPLES[key];
  if (_COMPARE_EXAMPLES[correctText]) return _COMPARE_EXAMPLES[correctText];
  // 从单词库取例句
  const d = getWordDetail(correctText);
  if (d.ex) return [{ en: d.ex, zh: d.exZh || "", note: d.mnemonic || "" }];
  return [];
}

function _getStudyTip() {
  return _STUDY_TIPS[Math.floor(Math.random() * _STUDY_TIPS.length)];
}

/**
 * 智能分析题目，生成多维度解析
 * @param {object} q - 题目对象
 * @param {number} correctIdx - 正确答案索引
 * @param {number|null} userIdx - 用户选择的索引
 * @returns {object} 分析结果
 */
function _analyzeQuestion(q, correctIdx, userIdx) {
  const prompt = q.prompt || "";
  const correctText = q.options && q.options[correctIdx] ? q.options[correctIdx] : "";
  const userText = (userIdx != null && q.options && q.options[userIdx]) ? q.options[userIdx] : "";

  // 1. 错误分类
  const errorInfo = _classifyError(prompt, correctText, userText, q.hint);

  // 2. 逐项解析
  const optionsAnalysis = (q.options || []).map((opt, i) => {
    const explanation = _getOptionExplanation(prompt, opt, i === correctIdx, q, errorInfo.label);
    return {
      text: opt,
      isCorrect: i === correctIdx,
      isUserChoice: i === userIdx,
      explanation: explanation,
    };
  });

  // 3. 对比例句
  const compareExamples = _getCompareExamples(correctText);

  // 4. 学习建议
  const studyTip = _getStudyTip();

  // 5. 为什么错了（针对性分析）
  let whyWrong = null;
  if (userText && userText !== correctText) {
    const userKey = userText.toLowerCase().trim();
    if (_OPTION_ANALYSIS[userKey] || _OPTION_ANALYSIS[userText]) {
      whyWrong = _OPTION_ANALYSIS[userKey] || _OPTION_ANALYSIS[userText];
    } else if (errorInfo.label.includes("条件句")) {
      whyWrong = `你选了 "${userText}"，但在虚拟语气条件句（If...would...）中，be 动词必须用 were。${userText} 的时态或形式不符合虚拟语气的要求。`;
    } else if (errorInfo.label.includes("被动")) {
      whyWrong = `你选了 "${userText}"，但被动语态需要 "be + 过去分词" 结构。${userText} 不是正确的被动形式。`;
    } else if (errorInfo.label.includes("完成时")) {
      whyWrong = `你选了 "${userText}"，但现在完成时需要 "have/has + 过去分词"。${userText} 不符合这一结构。`;
    } else if (errorInfo.label.includes("定语从句")) {
      whyWrong = `你选了 "${userText}"，但关系代词需要与先行词（人/物/地点）匹配。此处 "${userText}" 不匹配。`;
    } else if (errorInfo.label.includes("搭配")) {
      whyWrong = `"${userText}" 与句中其他词不构成正确搭配。正确搭配是 "${correctText}"。`;
    } else {
      whyWrong = `"${userText}" 在此处语境中不正确。正确答案是 "${correctText}"。`;
    }
  }
